...
#ifdef AUDIO_ENABLE
static Boolean flag_fPresentationTime = false;
#endif
void MPEGVideoStreamFramer::doGetNextFrame() {
  fParser->registerReadInterest(fTo, fMaxSize);
#ifdef AUDIO_ENABLE
  flag_fPresentationTime = false;
  continueReadProcessing(fPresentationTime);
#else
...
#ifdef AUDIO_ENABLE
  flag_fPresentationTime = true;
  framer->continueReadProcessing(presentationTime);
#else
...
#ifdef AUDIO_ENABLE
void MPEGVideoStreamFramer::continueReadProcessing(struct timeval presentationTime) {
  if(flag_fPresentationTime == true) {
//  	fPresentationTime = presentationTime;
  } else {
	presentationTime.tv_sec = 0;
	presentationTime.tv_usec= 0;
  }
  unsigned acquiredFrameSize = fParser->parse(presentationTime);
#else
