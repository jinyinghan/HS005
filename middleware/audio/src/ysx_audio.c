...
//#define USE_AEC_NO_HPF_NS_AGC   1
...
int QCamAudioInputOpen(QCamAudioInputAttr *pAttr) 
{
	...
	ret = QCamAudioInputSetVolume(90);
	if(ret != 0) {
	...
	if(ret != 0) {
		printf("Audio enable aec failed\n");
		return -1;
	}
		
	printf("xxxxxxxxxxxxx2\n");
 #ifndef USE_AEC_NO_HPF_NS_AGC
     ret = IMP_AI_EnableHpf(&attr);
     if(ret != 0) {
        printf("enable audio hpf error.\n");
        QLOG(FATAL, "enable audio hpf error.\n");
	...
	*/
	IMPAudioAgcConfig agcConfig = {
		.TargetLevelDbfs = 30,
		.CompressionGaindB = 40,
	};

	if(pAttr->sampleRate != AUDIO_SAMPLE_RATE_44100)
	{
	    ret = IMP_AI_EnableNs(&attr, NS_VERYHIGH);//Ωµ‘Î  
	    if(ret != 0) {
	        printf("enable audio ns error.\n");
	        QLOG(FATAL, "enable audio ns error.\n");
	        return -1;
		...
		ret = IMP_AI_EnableAgc(&attr, agcConfig);//øÿ÷∆“Ù¡ø
		if(ret != 0) {
...
//#endif
	...
	// ÊòØÂê¶ÂêØÁî®Âô™Â£∞ÊäëÂà∂,Âô™Â£∞ÊäëÂà∂ÁöÑmodeÂèÇÊï∞Ë°®Á§∫Âô™Â£∞ÊäëÂà∂ÁöÑÁ∫ßÂà´,ËåÉÂõ¥‰∏∫[0 ~ 3],Á∫ßÂà´Ë∂äÈ´ò,Âô™Â£∞ÊäëÂà∂ÁöÑË∂äÂπ≤ÂáÄ,ÁÑ∂ËÄå,Âô™Â£∞ÊäëÂà∂ÁöÑË∂äÂπ≤ÂáÄÂêåÊó∂‰πüÂ∞±‰ºö‰∏¢Â§±Êõ¥Â§öÁöÑÂ£∞Èü≥ÁªÜËäÇ,ÊâÄ‰ª•ËøôÈáåÊúâ‰∏Ä‰∏™ÁüõÁõæÁÇπ,ÈúÄË¶ÅÂú®
//#ifdef USE_NS
	printf("-------------audio input enable ns\n");
	ret = IMP_AI_EnableNs(&attr, NS_VERYHIGH);
	if(ret != 0) {
		printf("enable audio ns error.\n");
		return -1;
...
//#endif 
	...
	// ÊòØÂê¶ÂêØÁî®Ëá™Âä®Â¢ûÁõä
	printf("-------------audio input enable agc\n");
	IMPAudioAgcConfig agcConfig = {
		.TargetLevelDbfs = 30,
		.CompressionGaindB = 40,
	};
  ...
  //  sleep(1);
   if(record_tid && record_attr.start)
    ...
    if(devID)
    {
    	...
    	if(ret != 0) {
    		QLOG(-1, "Audio disable aec failed\n");
    	}        
#endif
...
int QCamAudioInputStart()
{
   ...
   /*
	...
	ret = IMP_AI_EnableAec(devID, chnID, 0, 0);
    if(ret != 0) {
         IMP_LOG_ERR(TAG, "IMP_AI_EnableAec failed\n");
         return NULL;
    }
	#endif
...
/*360Ω”ø⁄:Ù¡ø[0~100]£¨ -1 ±Ì æ π”√œµÕ≥ƒ¨»œ£¨≤ªΩ¯–––ﬁ∏ƒ
 *Ingenic ;[-30 ~ 120]. -30¥˙±Ìæ≤“Ù,120±Ì æΩ´…˘“Ù∑≈¥Û30dB,≤Ω≥§0.5dB. 60 «“Ù¡ø…Ë÷√µƒ“ª∏ˆ¡ŸΩÁµ„£¨
 */
...
int QCamAudioAecEnable(int enable)//ªÿ“Ùœ˚≥˝π¶ƒ‹
{
    int ret;
#ifdef NO_SPK
	return 0;
    ...
    if(enable){
        ...
        if(ret < 0)
            QLOG(TRACE, "Audio disable aec failed\n");                    
    }
#endif
    return ret;
}

int QCamAudioDenoiseEnable(int enable)//Ωµ‘Î
{
	int ret;
	IMPAudioIOAttr attr ;
...
int QCamAudioOutputPlay(char *pcm_data, int len)
{
	...
	while(1) 
    {
        if(has_left >= pb_attr.buf_size)  //¥Û”⁄“ª÷°
        {
