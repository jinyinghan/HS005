...
int main(int argc, char ** argv)
{
	...
	osd_attr.time_x = 0; //1280-19*16 = 976
    osd_attr.time_y = 0;
	...
	//sleep(2);
	osd_attr.time_x = 1464; //1280-19*16 = 976
    osd_attr.time_y = 0;	
    //ret = QCamVideoInput_SetJpgOSD(0, &osd_attr);
  	if(ret < 0){
   		printf("QCamVideoInput_SetOSD ERROR !\n");
        return -1;
   	}   
	...
	int ratio = 0;
	IMP_ISP_Tuning_GetDRC_Strength(&ratio);
	printf("ratio: %d\n",ratio);
	
	QCamVideoInput_SetBitrate(0, 1024, 1);
	QCamVideoInput_SetBitrate(1, 128, 0);
	#if 1
	QCamVideoInput_SetFps(0,12,2);
	QCamVideoInput_SetInversion(0);
	...
	*/
	...
	sleep(10);
	printf("test resolution start \n");
	//QCamVideoInput_SetResolution(0, QCAM_VIDEO_RES_1080P);
	//printf("xxxxxxxxxxxxxxxx2\n");
	sleep(2);
	//QCamVideoInput_SetCodec(0, 1);
	printf("test resolution end\n");
	sleep(2);
	Md2Snapshot();
	Md2Snapshot();
	sleep(5);
