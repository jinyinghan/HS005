...
struct chn_conf chn[FS_CHN_NUM] = {
	{
		.index = CH0_INDEX,
		.enable = CHN0_EN,
    	.payloadType = IMP_ENC_PROFILE_AVC_HIGH,//IMP_ENC_PROFILE_HEVC_MAIN,//
		.fs_chn_attr = {
			.pixFmt = PIX_FMT_NV12,
...
int sample_system_init()
{
	...
	if(ret != 0){
    	QLOG(FATAL, "IMP_ISP_Tuning_SetAntiFlickerAttr=%d failed !\n",flick_attr);
	}
#endif
...
int sample_encoder_init()
{
	...
	IMPFSChnAttr *imp_chn_attr_tmp;
	IMPEncoderChnAttr channel_attr;
	IMPEncoderRcMode S_RC_METHOD = IMP_ENC_RC_MODE_CAPPED_QUALITY;
    for (i = 0; i <  FS_CHN_NUM; i++) {
        if (chn[i].enable) {
            imp_chn_attr_tmp = &chn[i].fs_chn_attr;
            chnNum = chn[i].index;
...
int EncoderChannelAdd(QCamVideoInputChannel channel){
	...
	printf("step2 %d %d \n",chnNum,ch);
			...
			printf("step3 %d %d \n",chnNum,ch);
			ret = IMP_Encoder_RegisterChn(chn[ch].index, chnNum);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_Encoder_RegisterChn(%d, %d) error: %d\n", chn[ch].index, chnNum, ret);
				return -1;
			}
		}
			return 0;
...
#else
#if 0
extern int t_paylod[FS_CHN_NUM] ;
int EncoderChannelAdd(QCamVideoInputChannel channel){
	int i, ret, ch = 0, chnNum = 0;
	//IMPFSChnAttr *imp_chn_attr_tmp;
	...
	switch(channel.vbr)
	{
		...
		case 1:
			S_RC_METHOD = IMP_ENC_RC_MODE_VBR;break;
		default:
	      ...
	      // if (chn[ch].enable) {
            //imp_chn_attr_tmp = &chn[ch].fs_chn_attr;
            //chnNum = chn[ch].index;
			t_paylod[ch] = channel.payloadType;
            memset(&channel_attr, 0, sizeof(IMPEncoderChnAttr));
		    #if 1
...
int EncoderChannelAdd(QCamVideoInputChannel chn)
{
	...
	int S_RC_METHOD = IMP_ENC_RC_MODE_CAPPED_QUALITY;
