...
void video_stream_cb(int channel, const struct timeval *tv, const void *data, const int len, const int keyframe)
{
    int ret = 0;
	if (data && len > 0) {
		if (0 == channel) {		
//			printf("channel: %d, size: %d, timestamp: %d\n", channel, len, tv -> tv_sec * 1000 + tv -> tv_usec);
			ret = HisiPutH264DataToBuffer(data, len);
			if (ret < 0) {
				fprintf(stderr, "cache frame failed!\n");
			}
...
//	printf("size of %d",sizeof(HI_U16));
	...
	ringmalloc(2594*1922);//def:ringmalloc(720*576):720P, ringmalloc(1920*1280):1080P
	printf("RTSP server START\n");
	PrefsInit();
	printf("listen for client connecting...\n");
//	signal(SIGINT, IntHandl);
