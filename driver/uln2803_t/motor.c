...
//			printk("XXXXXXXXXXXXXXXXXXXX receive move cmd!\n");
		...
		case MOTOR_GET_STATUS:
		{
			...
			status.total_x_steps = info -> mattr[MOTOR_POS_DOWN].total_steps;
			status.total_y_steps = info -> mattr[MOTOR_POS_UP].total_steps;
			static int first = 1;
			if(first == 1){
				printk("%d %d\n",info -> mattr[MOTOR_POS_DOWN].cur_steps,info -> mattr[MOTOR_POS_UP].cur_steps);
				first = 0;
			}
			status.cur_x_steps = info -> mattr[MOTOR_POS_DOWN].cur_steps;//这个值0代表转到最下面
			status.cur_y_steps = info -> mattr[MOTOR_POS_UP].cur_steps;//这个值0代表转到最左边
...
static int motor_probe(struct platform_device *pdev)
{
	...
	jz_tcu_set_period(info->tcu, (24000000 /64/info->speed));//1500
