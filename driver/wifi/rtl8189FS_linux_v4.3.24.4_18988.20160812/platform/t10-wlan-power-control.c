...
/*
	...
	if (IS_ERR(power)) {
		printk("wifi_vbat regulator missing\n");
		goto err_put_vbat;
	}
	...
	if (IS_ERR(power)) {
		printk("wifi_vddio regulator missing\n");
		goto err_put_vddio;
	}
	ssv_6xxx_data.wifi_vddio = power;
*/

	printk("%s: start requset gpio	\n",__func__);
	printk("%s: start request gpio two\n",__func__);
	
	reset = GPIO_WIFI_RST_N;
	if ((pcgpio=gpio_request(reset, "wifi_reset"))) {
		printk("%s: no wifi_reset pin available!!\n",__func__); //pc@
		...
		printk("%s: gpio_direction_output\n",__func__); //pc @
	}
	
	printk("%s:gpio_request return %d\n",__func__,pcgpio); //pc@
    ...
    */
	if (gpio_request(GPIO_WIFI_WAKEUP, "wifi_wakeup")) {
		printk("ERROR: no wifi_wakeup pin available !!\n");
		goto err;
	} else {
...
/*
    if (wifi_wake_lock == NULL)
		printk("%s: invalid wifi_wake_lock\n", __func__);
	else if (wifi_vbat == NULL)
		printk("%s: invalid wifi_vbat\n", __func__);
	else if (wifi_vddio == NULL)
		printk("%s: invalid wifi_vddio\n", __func__);
	else if (!gpio_is_valid(reset))
		printk("%s: invalid reset\n", __func__);
	else
		goto start;
	return -ENODEV;
...
start:
    */
	...
	gpio_free(GPIO_WIFI_WAKEUP);
	gpio_free(GPIO_WIFI_RST_N);
	wake_unlock(wifi_wake_lock);
	//rtc32k_disable();
	return 0;
}
				...
				const char *buf, size_t count)
{
        ...
        if (strncmp(buf, "0", 1) == 0) {
                ...
                ret = -1;
        if (ret)
                printk("===>%s bcm_ap6212 power ctl fail\n", __func__);
